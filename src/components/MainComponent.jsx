...
function Main(props) {
  ...
  const featuredDish = store.dishes.filter(item => item.featured === true)[0];
  const featuredLeader = store.leaders.filter(
    ...
    <React.Fragment>
      <Routes>
        <Route path="/" element={<BasicLayout />}>
          ...
          />
          <Route path="menu" element={<Menu data={store.dishes} />} />
          <Route
            path="menu/:productId"
            element={
              <DishDetail data={store.dishes} comments={store.comments} />
            }
          />
          <Route path="Contact" element={<Contact />} />
          <Route path="About" element={<About leaders={store.leaders} />} />
        </Route>
      </Routes>
    </React.Fragment>
  );
